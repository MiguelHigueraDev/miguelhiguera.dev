      ...
      {!loading && statusMap[status.discord_status] ? (
        <>
          <div className="md:flex gap-2">
            <button
              data-tooltip-id="discord-id"
              data-tooltip-content="Discord: misfitdude"
              data-tooltip-place="bottom"
              className="status-button select-none rounded-xl py-2 px-3 gap-2 items-center inline-flex text-gray-300 bg-neutral-800 hover:bg-neutral-700 transition-colors duration-50 ease-in-out max-w-[300px] mt-3"
            >
              ...
              <div>
                ...
                >
                  ...
                  <span className="m-0 mr-1 p-0 text-center text-md select-none md:h-5">
                    {gameActivity.name.length < 25
                      ? gameActivity.name
                      : gameActivity.name.slice(0, 25) + "..."}
                  </span>
                  <Tooltip
                    id="game-playing"
                          ...
                          <>
                            {gameActivity.party ? (
                              <p className="text-sm md:text-md">
                                {gameActivity.state} (
                                {gameActivity.party.size[0]}/
                                {gameActivity.party.size[1]})
                              </p>
                            ) : (
                              <p className="text-sm md:text-md">
                                {gameActivity.state}
          ...
          {status.spotify && (
            <div>
              ...
              >
                ...
                <span className="m-0 mr-1 p-0 text-center text-md select-none md:h-5">
                  {status.spotify.artist.length < 25
                    ? status.spotify.artist
                    : status.spotify.artist.slice(0, 25) + "..."}{" "}
                  -{" "}
                  {status.spotify.song.length < 25
                    ? status.spotify.song
                    : status.spotify.song.slice(0, 25) + "..."}
                </span>
